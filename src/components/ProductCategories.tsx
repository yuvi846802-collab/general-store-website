        ...
        <AnimatePresence>
          {selectedCategory && (
            <motion.div
              ...
              id="product-list"
            >
              <div className="bg-[#0B0F19] rounded-[32px] border border-border shadow-2xl relative w-full p-6 md:p-10">
                
                {/* Top Bar: Close Button */}
                <div className="absolute top-6 left-6 md:top-8 md:left-8 z-30">
                  <button 
                    onClick={() => setSelectedCategory(null)}
                    className="w-10 h-10 rounded-full bg-[#1A1F2C] hover:bg-[#252B3B] flex items-center justify-center text-muted-foreground transition-colors shadow-lg"
                  ...
                  >
                    {currentProducts.map((product, idx) => (
                      <div key={idx} className={showAllItems ? "h-full" : "min-w-[280px] snap-start"}>
                        <div className="bg-[#131722] rounded-[24px] overflow-hidden border border-border hover:border-border transition-colors h-full flex flex-col group/card relative shadow-lg hover:shadow-2xl hover:-translate-y-1 duration-300">
                          
                          {/* Heart Icon */}
                          <button className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-[#131722]/60 backdrop-blur-md border border-border flex items-center justify-center text-muted-foreground hover:text-red-500 hover:bg-[#131722] transition-colors">
                ...
                {!showAllItems && (
                  <div className="flex items-center justify-center gap-2 mt-4">
                    {currentProducts.map((_, i) => (
                      <div 
                        key={i} 
                        className={`h-2 rounded-full transition-all duration-300 ${i === activeIndex || (activeIndex >= currentProducts.length && i === currentProducts.length - 1) ? 'w-6 bg-[#10b981]' : 'w-2 bg-accent/50'}`}
                      />
                    ))}
                  </div>
                )}
